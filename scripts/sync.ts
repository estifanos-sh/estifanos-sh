...
function convertMarkdownBodyToTypst(
  ...
  const { text: protectedText, placeholders } = protectCodeAndSpecialSyntax(markdown);
  ...
  // Images ![alt](src)
  result = result.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '#figure(image("$2"), caption: [$1])');
...
async function main() {
  ...
  try {
    ...
    files = entries.filter(
      (f) => typeof f === "string" && f.endsWith(".md") && !f.includes("templates/"),
    );
  } catch {
  ...
  const processFile = async (fileData: FileToProcess): Promise<SyncResult> => {
    ...
    try {
      ...
      execSync(`typst compile --root "${process.cwd()}" "${typstPath}" "${pdfPath}"`, {
        stdio: "pipe",
      });
      ...
      const uploadUrl = await client.mutation(api.journal.generateUploadUrl, {});
